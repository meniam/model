...
if ($run_as == 'groups') {
    ...
    foreach ($components as $component) {
        $component =   'ModelTest/' . basename(str_replace('_', '/', $component));

        if ($component == 'ModelTest/TestCase.php') {
            continue;
        }
        echo "$component:\n";

        //echo "$phpunit_bin $phpunit_opts $phpunit_coverage " . __DIR__ . '/' . $component;

        system("$phpunit_bin $phpunit_opts $phpunit_coverage --coverage-html html" . __DIR__ . '/' . $component, $c_result);
        echo "\n\n";
        if ($c_result) {
            $result = $c_result;
        }
...
function getAll($phpunit_conf)
{
    ...
    if ($handle = opendir(__DIR__ . '/ModelTest/')) {
