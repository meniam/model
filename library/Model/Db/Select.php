...
use \Model\Db\Exception\ErrorException as ErrorException;
use \Model\Db\Expr;
...
class Select
{
    ...
    public function columns($cols = '*', $correlationName = null)
    {
        ...
        if (!array_key_exists($correlationName, $this->_parts[self::FROM])) {
             ...
             * @see ErrorException
             */
            throw new ErrorException("No table has been specified for the FROM clause");
        }
    ...
    public function union($select = array(), $type = self::SQL_UNION)
    {
        ...
        if (!in_array($type, self::$_unionTypes)) {
            ...
            throw new ErrorException("Invalid union type '{$type}'");
        }
     ...
     * @return Select
     */
    public function where($cond, $bindParams = null)
    {
        if ($bindParams !== null && !is_array($bindParams)) {
            $bindParams = array($bindParams);
        }

        $this->_parts[self::WHERE][] = $this->_where($cond, $bindParams, true);
     ...
     * @param array    $bindParams
     * @internal param mixed $value OPTIONAL The value to quote into the condition.
     * @internal param int $type OPTIONAL The type of the given value
     * @return Select This Select object.
     *
     * @see      where()
     */
    public function orWhere($cond, array $bindParams = null)
    {
        $this->_parts[self::WHERE][] = $this->_where($cond, $bindParams,false);
     ...
     * @return Select This Select object.
     */
    public function having($cond, array $bindParams = null)
    {
        if ($bindParams !== null) {
     ...
     * @see      having()
     */
    public function orHaving($cond, array $bindParams = null)
    {
        if ($bindParams !== null) {
            $this->_bind = $bindParams;
        }
    ...
    public function getPart($part)
    {
        ...
        if (!array_key_exists($part, $this->_parts)) {
            throw new ErrorException("Invalid Select part '$part'");
        }
     ...
     * @return \PDOStatement
     */
    public function query(array $bind = null)
    {
        if ($bind === null) {
            $bind = $this->_bind;
        }

        return $this->_adapter->query($this->__toString(), $bind);
    }
    ...
    protected function _join($type, $name, $cond, $cols, $schema = null)
    {
        if (!in_array($type, self::$_joinTypes) && $type != self::FROM) {
            ...
            throw new ErrorException("Invalid join type '$type'");
        }

        if (count($this->_parts[self::UNION])) {
            
            throw new ErrorException("Invalid use of table with " . self::SQL_UNION);
        }
        ...
        if (!empty($correlationName)) {
            if (array_key_exists($correlationName, $this->_parts[self::FROM])) {
                ...
                throw new ErrorException("You cannot define a correlation name '$correlationName' more than once");
            }
    ...
    public function _joinUsing($type, $name, $cond, $cols = '*', $schema = null)
    {
        if (empty($this->_parts[self::FROM])) {
            ...
            throw new ErrorException("You can only perform a joinUsing after specifying a FROM table");
        }
     ...
     * @internal param string $type optional
     * @return string  clause
     */
    protected function _where($condition, array $bindParams = null, $bool = true)
    {
        if (count($this->_parts[self::UNION])) {
            throw new ErrorException("Invalid use of where clause with " . self::SQL_UNION);
        }
    ...
    public function renderOrder()
    {
        ...
        if (($order = $this->_renderOrder($sql))){
            $order = trim(substr($order, 9));
        }
    ...
    public function _renderLimit($sql, $count, $offset = 0)
    {
        $count = intval($count);
        if ($count <= 0) {
            throw new ErrorException("LIMIT argument count=$count is not valid");
        }

        $offset = intval($offset);
        if ($offset < 0) {
            throw new ErrorException("LIMIT argument offset=$offset is not valid");
        }
     ...
     * @return Select
     * @throws ErrorException If an invalid method is called.
     */
        ...
        if (preg_match('/^join([a-zA-Z]*?)Using$/', $method, $matches)) {
            ...
            if ($type) {
                ...
                if (in_array($type, array(self::CROSS_JOIN, self::NATURAL_JOIN))) {
                    ...
                    throw new ErrorException("Cannot perform a joinUsing with method '$method()'");
                }
            } else {
     ...
     * @var string
     */
    const ROW_COUNT_COLUMN = 'select_row_count';
